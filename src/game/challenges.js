// Challenge phase — initiation, selection, resolution, reveal
// Handles the full challenge flow from pre-reveal through resolution

import { escapeHtml, createDebounce } from '../utils.js';
import { Phase, transition } from './phases.js';

const _nextTurnDebounce = createDebounce(300);
...
export const challengeMethods = {
    ...
    startChallenge() {
        ...
        otherPlayers.forEach(({ player, index }) => {
            playerButtons += `<button class="challenge-player-btn" data-action="select-challenger" data-player="${index}">${escapeHtml(player.name)} (\u{1F536}${player.tokens})</button>`;
        });
    ...
    // ─── Shared: refund current challenger and remove from list ───

    _refundCurrentChallenger() {
        const cp = this.challengePhase;
    ...
    renderChallengerTimeline() {
        ...
        el.innerHTML = this._renderTimelineHTML(originalPlayer, true, 'challenger-drop-zone', disabledSet);
